...
# Deterministic ownership marker for every replaceable tool-authored diagnostic
# sheet. Preserve-formatting output may delete/recreate a sheet only when A1
# begins with this marker.
TOOL_SHEET_MARKER = "Generated by Reliability Tools"


def _owned_sheet_banner(explanation: str) -> str:
    return f"{TOOL_SHEET_MARKER} - {explanation}"
...
NEW_REFDES_BANNER = _owned_sheet_banner(
...
PART_USAGE_DIAGNOSTICS_BANNER = _owned_sheet_banner(
...
SUMMARY_SHEET_BANNERS: Dict[str, str] = {
    "No_Matches": _owned_sheet_banner(
        "Components for which no failure mode could be generated."
    ),
    "Missing_HDA": _owned_sheet_banner(
        "Components missing an HDA commodity assignment."
    ),
    "Group_Missing_BOM": _owned_sheet_banner(
        "Function-group RefDes values that were not present in the BOM."
    ),
    "BOM_Missing_Refs": _owned_sheet_banner(
        "BOM rows whose Reference Designator value was missing or unreadable."
    ),
    "BOM_Duplicate_Refs": _owned_sheet_banner(
        "Reference Designators that occurred more than once in the BOM."
    ),
    "Validation_Warnings": _owned_sheet_banner(
        "Failure Mode Ratio and Part Usage validation warnings for review."
    ),
...
def prepend_tool_sheet_banner(ws, banner: str, column_count: int) -> None:
    """Prepend an ownership banner and pin it with the header row."""
...
def write_excel_report(
    ...
    for name, frame in summaries.items():
        if not frame.empty:
            ...
            # Every replaceable diagnostic sheet starts with the deterministic
            # ownership marker; explanatory tails remain sheet-specific.
