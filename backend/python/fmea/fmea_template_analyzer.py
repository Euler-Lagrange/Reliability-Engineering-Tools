...
WorkbookFingerprint = Tuple[int, int]
...
class FunctionGroup:
    ...
    pp_styles: Dict[int, CellStyle] = field(default_factory=dict)
    ...
    for idx, clf in enumerate(classifications):
        ...
        if clf.row_type == 'circuit_block':
            ...
            if current_group.pp_start_row == 0:
                ...
                # Capture a prototype for every analyzed column.  The writer
                # falls back to column A only for columns it appends later.
                current_group.pp_styles = {
                    column: capture_cell_style(ws.cell(row=row_num, column=column))
...
_TARGET_CHANGED_MESSAGE = (
    "The target workbook changed on disk while the merge was running "
    "(is it open in Excel or syncing?). No output was written — close it and re-run."
)


def capture_workbook_fingerprint(
    workbook_path: str | Path,
) -> WorkbookFingerprint:
    """Return the identity fields used to detect an in-flight source change."""
    source_stat = Path(workbook_path).stat()
    return source_stat.st_mtime_ns, source_stat.st_size


def verify_workbook_fingerprint(
    workbook_path: str | Path,
    expected: WorkbookFingerprint,
) -> None:
    """Fail closed when the preserve target no longer matches analysis input."""
    try:
        current = capture_workbook_fingerprint(workbook_path)
    except OSError:
        current = None
    if current != expected:
        raise ValidationError(_TARGET_CHANGED_MESSAGE)


def _analyze_loaded_workbook(
    wb: Workbook,
    workbook_path: str,
    source_fingerprint: WorkbookFingerprint,
    *,
    ...
    cancel_token,
    log_func: Any,
    failure_modes_standard: str,
    package_has_images_or_shapes: bool,
    package_has_charts: bool,
) -> TemplateMap:
    """Analyze an owned workbook; the caller retains close responsibility."""
    if cancel_token:
        cancel_token.check()
    ...
    log_func(f"FMEA sheet detected: '{fmea_sheet_name}'")

    if cancel_token:
        cancel_token.check()

    header_row = _detect_header_row(ws)
    log_func(f"Header row detected: row {header_row}")
    ...
    # pandas opens the path a second time.  Refuse a stale analyzer map before
    # that re-read can combine two different workbook versions.
    verify_workbook_fingerprint(workbook_path, source_fingerprint)
    df = try_read_table(
        str(workbook_path),
    ...
    log_func(f"Read {len(df)} data rows from '{fmea_sheet_name}'")

    if cancel_token:
        cancel_token.check()

    classifications, _level_col, _level_validated = classify_fmea_rows(
        df, log_func=log_func, cancel_token=cancel_token,
    )
    ...
    if row_offset != 0:
        ...
        log_func(
            f"Adjusted row indices by +{row_offset} for header at row {header_row}"
        )
...
def analyze_template(
    ...
    try:
        source_fingerprint = capture_workbook_fingerprint(workbook_path)
    except OSError as exc:
        raise FileAccessError(
            f"Cannot inspect workbook: {exc}",
    ...
    try:
        template_map = _analyze_loaded_workbook(
            wb,
            workbook_path,
            source_fingerprint,
            ...
            log_func=_log,
            failure_modes_standard=failure_modes_standard,
            package_has_images_or_shapes=package_has_images_or_shapes,
            package_has_charts=package_has_charts,
        )
