...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 404 backend tests (52 sidecar + 27 audit + 12 cancel bridge + 10 output-directory helper + 77 FMEA phase D + 27 failure-rate logic + 16 RefDes extraction-engine + 42 BOM-compare logic + 14 BOM-compare runtime + 9 extraction-compare + 4 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 12 NextGen engine + 29 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
...
- 16 RefDes extraction-engine tests in `test_extraction_engine.py` (pin-disambiguation, bounded word extraction across both group-fallback probes, Tier-2 #20 pinlist-failure streaming, and the geometry RefDes-check prefix-allowlist alignment (#6) + `pin_assignment_threshold` key + cap-hit warning)
- 42 BOM-compare logic tests in `test_bom_compare_logic.py` (incl. custom-path option semantics, residual-digit guard, non-numeric FMR flagging, per-column value-diff contract, Tier-4 FMR-key canonicalization against invisible-char RefDes, the Batch-5 custom FMR sheet user-facing status translation, Batch-6 friendly ValidationError messages for empty/wrong-sheet inputs, the final-day fixes: group-report expander scoped to tool-authored columns so user text ships verbatim, detect_column tolerating numeric Excel headers, and the UX round-2 pair: content-based FMEA detection — validated-level-column trigger without filename, explicit-evidence gate so plain BOMs never misfire, filename fallback unchanged — plus the custom-path space-scheme sheet names with a no-underscore lockstep guard)
- 14 BOM-compare runtime tests in `test_bom_compare_runtime.py` (dnp_regex default, Do-Not-Map sentinel validation, custom option wiring, compare_columns forwarding, cancellation before promote in group/custom/extraction-compare paths, Batch-6 display-label blocked-run messages)
...
- 4 Failure-Rate runtime tests in `test_failure_rate_runtime.py` (Do-Not-Map sentinel validation, cancellation before promote, Batch-6 display-label blocked-run messages)
...
- 12 NextGen-engine tests in `test_nextgen_engine.py` (default engine: pinlist-failure streamed to the run log; word-extraction-timeout surfacing; pin-token verification against pin-level BOM entries incl. the Verified-row formatting; diagnostics accumulator: None is a no-op, token-diag merge skips None fields, pinned orphan-disposition vocabulary, token-pages fold with mode-suffix strip + cross-group duplicate group listing, per-pass adoption replaces rather than merges; Batch-5: checkpoint write failure degrades to a warning, geometry checkpoints are opt-in)
- 29 RefDes-runtime tests in `test_refdes_runtime.py` (output DataFrame drops internal `_is_gap`/`_row_style` columns and renames to Title Case display headers; blocking type/range validation of all 16 engine options at validate time, incl. the 0-valid adaptive-orphan threshold and a config-field lockstep guard; cancellation after engine return, during group detection, and before output promotion; Batch-5: verified-group metrics require components not just the row-label suffix, out_folder plumbed for geometry checkpoints, sidecar main() installs multiprocessing.freeze_support)
...
### Frontend Tests (306 total across 46 test files)
...
- `src/components/InputGrid.test.tsx` (new — per-card copy-path state isolation)
...
see individual counts per file — the suite totals 306 tests and changes
