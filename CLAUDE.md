...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 347 backend tests (49 sidecar + 27 audit + 12 cancel bridge + 10 output-directory helper + 71 FMEA phase D + 24 failure-rate logic + 15 RefDes extraction-engine + 31 BOM-compare logic + 9 BOM-compare runtime + 9 extraction-compare + 2 failure-rate runtime + 6 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 3 crash-dump + 10 NextGen engine + 24 RefDes runtime + 12 validation-notes)
