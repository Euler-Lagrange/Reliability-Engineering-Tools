...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 393 backend tests (50 sidecar + 27 audit + 12 cancel bridge + 10 output-directory helper + 75 FMEA phase D + 27 failure-rate logic + 15 RefDes extraction-engine + 42 BOM-compare logic + 11 BOM-compare runtime + 9 extraction-compare + 3 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 12 NextGen engine + 27 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
...
- 27 RefDes-runtime tests in `test_refdes_runtime.py` (output DataFrame drops internal `_is_gap`/`_row_style` columns and renames to Title Case display headers; blocking type/range validation of all 16 engine options at validate time, incl. the 0-valid adaptive-orphan threshold and a config-field lockstep guard; Batch-5: verified-group metrics require components not just the row-label suffix, out_folder plumbed for geometry checkpoints, sidecar main() installs multiprocessing.freeze_support)
