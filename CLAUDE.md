...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 388 backend tests (50 sidecar + 27 audit + 12 cancel bridge + 10 output-directory helper + 75 FMEA phase D + 27 failure-rate logic + 15 RefDes extraction-engine + 37 BOM-compare logic + 11 BOM-compare runtime + 9 extraction-compare + 3 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 12 NextGen engine + 27 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
