...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 498 backend tests (52 sidecar + 27 audit + 12 cancel bridge + 18 output-directory helper + 119 FMEA phase D + 1 FMEA template analyzer + 8 FMEA column-resolution + 27 failure-rate logic + 22 RefDes extraction-engine + 57 BOM-compare logic + 16 BOM-compare runtime + 10 extraction-compare + 4 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 16 NextGen engine + 36 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
...
- 8 FMEA column-resolution tests in `test_fmea_column_resolution.py` (2026-07-20 "BAE PN" field failure: BAE PN registered as a part-number synonym, word-boundary substring matching for short synonyms in `resolve_column` — "Customer PN"/"Mfr P/N" resolve while "PNP Driver" and the original Ref-vs-Reference-Designator guard stay rejected — with exact-match and long-synonym behavior pinned unchanged)
...
### Frontend Tests (347 total across 47 test files)
...
see individual counts per file — the suite totals 347 tests and changes
