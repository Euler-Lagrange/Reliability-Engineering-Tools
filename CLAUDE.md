...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 472 backend tests (52 sidecar + 27 audit + 12 cancel bridge + 18 output-directory helper + 118 FMEA phase D + 1 FMEA template analyzer + 27 failure-rate logic + 22 RefDes extraction-engine + 42 BOM-compare logic + 14 BOM-compare runtime + 10 extraction-compare + 4 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 16 NextGen engine + 36 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
...
- 36 RefDes-runtime tests in `test_refdes_runtime.py` (output DataFrame drops internal `_is_gap`/`_row_style` columns and renames to Title Case display headers; blocking type/range validation of all 17 engine options at validate time, incl. the 0-valid adaptive-orphan threshold and a config-field lockstep guard; cancellation after engine return, during group detection, and before output promotion; zombie word threads prevent an unsafe PDF close while the zero-survivor path closes normally; Batch-5: verified-group metrics require components not just the row-label suffix, out_folder plumbed for geometry checkpoints, sidecar main() installs multiprocessing.freeze_support; Wave 3: BOM cross-check soft-fail feeds warning_count; Wave R: the DIG-4xx combined silent-loss regression, `annotation_page_timeout_seconds` as the 17th lockstep-validated option with timed-out pages qualifying the toast, and bom-collision kept-and-flagged note excluded from dropped-pin counts)
