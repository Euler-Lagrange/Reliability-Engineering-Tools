...
.venv\Scripts\python.exe -m pytest backend/tests -v    # 489 backend tests (52 sidecar + 27 audit + 12 cancel bridge + 18 output-directory helper + 119 FMEA phase D + 1 FMEA template analyzer + 27 failure-rate logic + 22 RefDes extraction-engine + 56 BOM-compare logic + 16 BOM-compare runtime + 10 extraction-compare + 4 failure-rate runtime + 9 read-layer + 19 RefDes BOM-coverage + 6 BOM-loader + 5 OneDrive detection + 3 file-size guard + 4 crash-dump + 16 NextGen engine + 36 RefDes runtime + 12 validation-notes + 7 refdes-prefix config + 3 excel-styles + 5 validate-output-path)
