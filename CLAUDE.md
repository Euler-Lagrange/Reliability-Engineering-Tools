...
- 18 RefDes extraction-engine tests in `test_extraction_engine.py` (pin-disambiguation, bounded word extraction across both group-fallback probes, post-join zombie counting and survivor retention, Tier-2 #20 pinlist-failure streaming, and the geometry RefDes-check prefix-allowlist alignment (#6) + `pin_assignment_threshold` key + cap-hit warning)
...
- 31 RefDes-runtime tests in `test_refdes_runtime.py` (output DataFrame drops internal `_is_gap`/`_row_style` columns and renames to Title Case display headers; blocking type/range validation of all 16 engine options at validate time, incl. the 0-valid adaptive-orphan threshold and a config-field lockstep guard; cancellation after engine return, during group detection, and before output promotion; zombie word threads prevent an unsafe PDF close while the zero-survivor path closes normally; Batch-5: verified-group metrics require components not just the row-label suffix, out_folder plumbed for geometry checkpoints, sidecar main() installs multiprocessing.freeze_support)
...
### Frontend Tests (312 total across 46 test files)
...
see individual counts per file — the suite totals 312 tests and changes
