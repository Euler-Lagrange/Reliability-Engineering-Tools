...
| Frontend BOM Compare tool | `frontend/src/features/bom-compare/BomCompareTool.test.tsx` | 25 | Vitest + RTL |
...
| Read layer (in-process, 9 tests) | Excel/CSV `NA`/`N/A` literal-text read parity; duplicate-header dedup matching pandas' `.1`/`.2` scheme; NaN-safe RefDes-family canonicalization; invisible-character prefix cleanup; and non-shared style-array stamps |
...
| BOM-loader metadata (in-process, 6 tests) | Opt-in Part Number / Description capture in `load_bom_data`, keyed to normalized RefDes with Description-over-Name precedence; pin-style rows count as their base component; metadata collection remains off by default |
...
| `frontend/src/components/ContextDrawer.test.tsx` | 5 — active-tool run summary and preview, tool-specific empty state, closed-drawer tab order, Escape handling, top-layer-only dismissal |
| `frontend/src/shared/hooks/useEscapeLayer.test.ts` | 4 — top-layer-only Escape dismissal, unregister-on-unmount fallback, no-layer behavior, non-Escape keys |
| `frontend/src/shared/errors/ErrorBoundary.test.tsx` | 3 — fallback rendering plus inline diagnostic-copy success and failure states |
...
| `frontend/src/components/CustomSelect.test.tsx` | 4 — keyboard navigation, opt-in empty-value placeholder, no-placeholder default, scrollable viewport class wiring |
| `frontend/src/components/MappingTable.test.tsx` | 15 — column mapping display, selection, validation, sync, source-aware option labels, required-column marker, valid explicit overrides reported as Manual, orphaned overrides retained as unmapped, optional-unmapped rows neutral + excluded from the unmapped badge |
| `frontend/src/components/RunStatePanel.test.tsx` | 6 — run state display, progress, result, cancel, error |
| `frontend/src/components/OutputFolderPicker.test.tsx` | 2 — native-dialog raw-string and `Error` rejection surfacing |
| `frontend/src/components/GlobalLogPanel.resize.test.tsx` | 13 — log panel drag-to-resize, collapse, expand, boundary constraints |
| `frontend/src/components/primitives/CommandPalette.test.tsx` | 8 — open, search, select, keyboard navigation, dismissal, and current command wiring |
...
| `frontend/src/shared/hooks/useAppShortcuts.test.tsx` | 6 — platform-specific tool switching, editable-target guard, Review drawer toggle, documented theme shortcuts, AltGr guard |
...
| `frontend/src/stores/notificationStore.test.ts` | 3 — capacity eviction for sticky errors and ordinary notifications |
| `frontend/src/stores/storeMigrations.test.ts` | 7 — theme and shell persisted-state migrations, sanitization, defaults, and transient-state exclusion |
...
| `frontend/src/features/fmea/FmeaTool.inspection.test.tsx` | 6 — sheet selection interactivity during background aggregation, inspection cap warning display, stale-validation clearing across configuration changes, orphaned-override pruning |
...
| `frontend/src/features/bom-compare/BomCompareTool.test.tsx` | 25 — custom slots, workflow-local onboarding/cache behavior, inspection recovery, run guards and error surfacing, header-derived mapping, dispatch options, display-name inputs, custom column pairs, selector locking, cross-tool inspection pause |
| `frontend/src/features/failure-rate/FailureRateTool.test.tsx` | 5 — stale-validation clearing, real-header mapping derivation, cross-tool inspection pause, and onboarding conformance |
| `frontend/src/features/refdes-extractor/RefDesExtractorTool.test.tsx` | 17 — piece-part pinlist slot, pristine behavior, dispatch and inspection guards, adaptive geometry, numeric tuning, advanced controls, and annotation-page timeout |
...
| `frontend/src/features/toolRunDispatch.test.tsx` | 26 — FMEA (option-key payload incl. hdaSource), BOM Compare, Failure Rate, and RefDes workflow dispatch from React tools, validation-failure (execute_run skipped), all six BOM Compare options reach the backend, Custom Compare column pairs reach `options.compare_columns`, desktop mode never seeds demo validation content (M9), FMEA Required indicators on mandatory input roles, the cross-tool run guard (blocks + toasts while another tool's run is live; terminal runs don't block), exact bridge execute-timeout acceptance-unknown warnings across all four tools, late-ack run preservation across the timeout catch, numeric Run-rail progress across all four tools, stale deferred-validation suppression after configuration changes with a fresh-run recovery, and a negative assertion that the corresponding validate timeout remains an error |
| `frontend/src/components/WorkflowSelector.test.tsx` | 4 — button semantics, labelled toggle group, selection callback, disabled state |
| `frontend/src/components/StrategySelector.test.tsx` | 4 — button semantics, labelled toggle group, selection callback, disabled state |
| `frontend/src/components/InputGrid.test.tsx` | 3 — independent path-copy state, optional display-name editing, and inspection-control disabled reasons |
| `frontend/src/features/settings/SettingsTool.test.tsx` | 7 — RefDes prefix load/edit/save/validation/retry/browser-preview behavior and the in-app User Guide |
| `frontend/src/app/App.shellHooks.test.tsx` | 1 — shell-level bootstrap and run-subscription hook installation |
| `frontend/src/mocks/scenarios.test.ts` | 2 — scenario input-slot completeness across tools and BOM Custom Compare |
| `frontend/src/components/ValidationPreview.test.tsx` | 5 — compact preview, Review drawer affordance, empty state, and severity-aware issue counts |
