...
| Backend FMEA Phase D | `backend/tests/test_fmea_phase_d.py` | 77 | pytest |
| Backend Failure-Rate logic | `backend/tests/test_failure_rate_logic.py` | 27 | pytest |
| Backend RefDes extraction-engine | `backend/tests/test_extraction_engine.py` | 16 | pytest |
...
| Backend RefDes runtime | `backend/tests/test_refdes_runtime.py` | 29 | pytest |
...
| Frontend BOM Compare tool | `frontend/src/features/bom-compare/BomCompareTool.test.tsx` | 21 | Vitest + RTL |
...
| RefDes extraction-engine (in-process, 16 tests) | `_disambiguate_pin_mapping` pin-label collision resolution across the three-tier priority (body center inside group rect → body overlaps rect → nearest body by distance); bounded word extraction across both group-fallback probes; the Tier-2 #20 pinlist-failure run-log surfacing; and the geometry RefDes-check prefix-allowlist alignment (#6), the `pin_assignment_threshold` config key, and cap-hit run-log warnings |
...
| RefDes runtime (in-process, 29 tests) | Runtime option validation, output projection, and cancellation checks during group detection, after extraction, and before output promotion |
