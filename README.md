...
.venv\Scripts\python.exe -m pytest backend\tests -v   # Backend tests (265 tests across sidecar, audit, cancel bridge, output-directory helpers, FMEA phase D, failure-rate logic, RefDes extraction-engine, BOM-compare logic, read-layer, RefDes BOM-coverage, BOM-loader metadata, OneDrive detection, file-size guard, crash-dump)
```
